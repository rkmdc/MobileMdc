...
TriggerClick_for_left_menu = 0;
TriggerClick_for_right_menu = 0;
...
MainMenuObj.prototype.setupMenuSlider = function() {
    $('.main-menu-button-left a.menu').click(function(e) {
        e.preventDefault();
//		$(".app-wrapper").toggleClass("leftmenu-open");
        if (TriggerClick_for_left_menu === 0)
        {
            TriggerClick_for_left_menu = 1;
            $('.app-wrapper').animate({left: '87%'}, 200, function() {
                $('.app-wrapper').css('width', '13%');
                $('.Round_Blue_Canvas').removeClass('show').addClass('hide');
            });

            $('header').animate({left: '87%'}, 200);
            $('nav.main-left-menu').css("z-index", "2");
        } else
        {
            TriggerClick_for_left_menu = 0;
            $('.app-wrapper').css('width', '100%');
            $('.app-wrapper').animate({left: '0%'}, 200, function() {
                $('.app-wrapper').css('left', '');
                $('nav.main-left-menu').css("z-index", "-1");
                 
            });
            $('header').animate({left: '0%'}, 200, function() {
                $('header').css('left', '');
            });
               $('.Round_Blue_Canvas').removeClass('hide').addClass('show');
//             $('.app-wrapper').removeAttr("style");


        }
    });
    $('.main-menu-button-right a.gear').click(function(e) {
        e.preventDefault();
//        $(".app-wrapper").toggleClass("rightmenu-open");
        if (TriggerClick_for_right_menu === 0)
        {
            TriggerClick_for_right_menu = 1;
            $('.app-wrapper').animate({right: '87%'}, 200, function() {
             
                $('.Round_Blue_Canvas').removeClass('show').addClass('hide');
            });
            $('header').animate({right: '87%'}, 200);
        } else
        {
            TriggerClick_for_right_menu = 0;
              $('.app-wrapper').css('width', '100%');
            $('.app-wrapper').animate({right: '0%'}, 200, function() {
                $('.app-wrapper').css('right', '');
            });
            $('header').animate({right: '0%'}, 200, function() {
                $('header').css('right', '');
            });
              $('.Round_Blue_Canvas').removeClass('hide').addClass('show');
//             $('.app-wrapper').removeAttr("style");
...
FormObj.prototype.renderProgressDotted = function() {
    ...
    $.each($('.progress-dotted'), function() {
        ...
        if (labelLeft < 0)
            labelLeft = 0;
        if (labelLeft > ww - labelEl.width())
            labelLeft = ww - labelEl.width();
        labelEl.css({
            left: labelLeft
        });
        $('ul', this).remove();
...
MapObj.prototype.bindInfoWindowToMarker = function(marker, contentString, infowindow) {
    ...
    google.maps.event.addListener(marker, 'click', function() {
        if (t.openInfowindow != null)
            t.openInfowindow.close();
...
MapObj.prototype.rad = function(x) {
...
ReportObj.prototype.renderBarGraph = function() {
    ...
    d3.json("json/products.json", function(error, result) {
        ...
        t.axis.x.domain(t.data.map(function(d) {
            return d.xAxisIndex;
        }));
        t.axis.y.domain([0, d3.max(t.data, function(d) {
                return d.variable;
            })]);
        ...
        t.svg.selectAll(".bar")
                ...
                .attr("class", "bar")
                .attr("x", function(d) {
                    return t.axis.x(d.xAxisIndex);
                })
                .attr("width", t.axis.x.rangeBand())
                .attr("y", function(d) {
                    return t.axis.y(d.variable);
                })
                .attr("height", function(d) {
                    return t.height - t.axis.y(d.variable);
                });
...
ReportObj.prototype.renderDonutGraph = function() {
    ...
    d3.json(t.jsonURL, function(error, result) {
        ...
        t.svg.selectAll(".legend-box")
                ...
                .attr("y", function(d, i) {
                    return t.height / 2 + (t.legendBoxSize + t.legendBoxSpacing) * i;
                })
        ...
        t.svg.selectAll(".legend-text")
                ...
                .attr("x", function(d) {
                    return -legendLeft + (t.legendBoxSize + t.legendBoxSpacing);
                })
                .attr("y", function(d, i) {
                    return t.height / 2 + (t.legendBoxSize + t.legendBoxSpacing) * i;
                })
...
ReportObj.prototype.renderLineGraph = function() {
    ...
    d3.json(t.jsonURL, function(error, result) {
        ...
        t.axis.y = d3.scale.linear()
                .domain([0, d3.max(t.data, function(d) {
                        return d.total;
                    })])
        ...
        t.line.valueline = d3.svg.line()
                .x(function(d) {
                    return t.axis.x(new Date(d.date));
                })
                .y(function(d) {
                    return t.axis.y(d.total);
                });
